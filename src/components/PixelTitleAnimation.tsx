...
export default function PixelTitleAnimation({
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const pixelsToDrawThisFrame = Math.ceil((animationSpeed * elapsed) / 1000)
      let newPixelsDrawn = 0

      // Draw new pixels
      ctx.fillStyle = color

      for (let i = 0; i < pixelsToDrawThisFrame && pixelsDrawn + i < totalPixels; i++) {
  ...
  }, [text, fontSize, fontFamily, color, backgroundColor, windowSize, onAnimationComplete, pixelSize, animationSpeed])
